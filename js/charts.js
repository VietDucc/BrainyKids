function index_to_shortname( index ){
    return ["x_frame_options","interesting_responses","insecure_cookies"][index];
}

function index_to_severity( index ){
    return {"x_frame_options":"low","interesting_responses":"informational","insecure_cookies":"informational"}[index_to_shortname(index)];
}
...
function renderCharts() {
    ...
    c3.generate({
        ...
        axis: {
            x: {
                type: 'category',
                categories: ["Missing 'X-Frame-Options' header","Interesting response","Insecure cookie"],
                tick: {
                    rotate: 15
                }
            },
    ...
    c3.generate({
        ...
        data: {
            type: 'pie',
            columns: [["Trusted",10],["Untrusted",0]]
        },
        pie: {
